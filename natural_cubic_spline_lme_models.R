...
(alsp_ns_f_pred <- data.frame(age = seq(min(alsp_f$age), max(alsp_f$age), length = 50)))
...
(alsp_ns_m_pred <- data.frame(age = seq(min(alsp_m$age), max(alsp_m$age), length = 50)))
...
(bmdcs_ns_f_pred <- data.frame(age = seq(min(bmdcs_f$age), max(bmdcs_f$age), length = 50)))
...
(bmdcs_ns_m_pred <- data.frame(age = seq(min(bmdcs_m$age), max(bmdcs_m$age), length = 50)))
...
(pbmas_ns_f_pred <- data.frame(age = seq(min(pbmas_f$age), max(pbmas_f$age), length = 50)))
...
(pbmas_ns_m_pred <- data.frame(age = seq(min(pbmas_m$age), max(pbmas_m$age), length = 50)))
...
nspline_plot_fun <- function(data1, data2){ ggplot(
  data = data1, aes(x = age, y = tblh_bmc_, ymin = tblh_bmc_-(1.96*se), ymax = tblh_bmc_+(1.96*se))) +
    geom_line(aes(col = "Males"), data = data1) + geom_line(aes(col = "Females"), data = data2) + theme_bw() +
    geom_ribbon(data = data1, alpha=0.1) + geom_ribbon(data = data2, alpha=0.1) +
...
nspline_vel_plot_fun <- function(data1, data2, apv1, apv2){ ggplot(
  data = data1, aes(x = x, y = y)) +
    ...
    geom_vline(xintercept = apv2, linetype="dotted", color = "red2") + theme_bw() +
    scale_colour_manual(name="legend", values=c("red2", "black")) + 
    scale_x_continuous(breaks=c(4, 8, 12, 16, 20, 24, 28, 32, 36, 40), limits=c(4, 40)) + 
